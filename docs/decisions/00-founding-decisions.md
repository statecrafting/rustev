...
### Owner decisions of 2026-09-24, the remote backend continued

Decided by the owner on 2026-09-24 in answer to the open questions of
drafts `009` and `012`, as relayed in the work order that applied them.
Recorded here as the owner's decisions, except where a row names an
agent-selected default.

| Id | Decision |
|---|---|
| R-27 | O-02 is closed. Owner's words: "O-02, paid Jev inference: authorized and the Vercel AI_GATEWAY_API_KEY has been added to "/Users/bart/.config/statecrafting/infra/vercel/.env"". Scope: paid Jev inference through the Vercel AI Gateway for the smoke and qualification stages of spec `012`. The key is read from that file at runtime by the host or test harness and is never copied into any repository, log, fixture or evidence record. Every request sets `providerOptions.gateway{zeroDataRetention: true, only: ["typesafe-ai"]}`. Data that may be sent: the travel-memory synthetic fixture corpus and independently labeled evaluation data (R-28); no real user mail until a separate privacy decision. The owner set no spend cap. **Agent-selected default:** USD 25 per calendar month for development and qualification together, enforced by a host-held shared cost ledger in the harness (spec `003`, 3.5.2) plus a Gateway dashboard budget; the adapter's charges are estimates, so the ledger is not a guaranteed cap and the dashboard budget is the backstop. The owner may change it. This decision authorizes no approval or implementation of specs `009` or `012` and no production use; the change that records it makes no live call. |
| R-28 | The owner accepted the recommendations: (1) `rank` stays composed in plans, with no native or adapter-composed operation; (2) `served: unknown` on the Gateway is acceptable for development only; production uses TypeSafe's direct API with a pinned version unless Gateway pinning is verified and recorded; (3) adapter-level shared-state batching (spec `009`, 3.10.4) is the optimization step R-08 deferred, for remote adapters, off by default until qualified, and its cost attribution (shares to members that received the response, liability for the rest until the host reconciles) is accepted; (4) the dropped-connection gap (spec `009`, 5.1) is proposed as a separately reviewable amendment of spec `003` (draft spec `013`), not accepted as a limitation; (5) exchange records go to a separate host sink now, and a `rustev.run/2` member is a later change; (6) option descriptions stay in the adapter binding for now. For R-04, travel-memory supplies an independently labeled evaluation set of synthetic provenance, which may be sent to the provider under zero data retention. R-04 still applies to what it can show: quality measured on it is stated as measured on synthetic-provenance content, not as evidence about real user data. |
| R-29 | Spend caps, set by the owner on 2026-09-24, replacing R-27's agent-selected default (R-27's row is kept as history). Owner's words: "25$ for production grade system is fine, testing should be capped at $5". Caps: USD 25 per calendar month for production use of the Jev integration; USD 5 in total, not per month, for testing (smoke, qualification and evaluation runs together). Enforcement: the adapter's cost accounting stops before dispatch any attempt whose reservation would take the cumulative spend of its budget over the cap (a hard stop, spec `003` 3.5.3 semantics applied to a shared ledger that persists across runs), plus a Gateway dashboard budget. Because the adapter's per-call amounts are estimates, the hard stop acts on estimates and on observed charges as they arrive; the dashboard budget is the backstop for the difference. This row sets the production cap; it does not approve spec `012` or authorize a production deployment, which remain the owner's separate acts, and production still requires a pinned transport (R-28). |

### Owner decisions of 2026-09-24, approval and the Gateway plan

Decided by the owner on 2026-09-24. Recorded here as the owner's decisions.
...
None. O-01 was closed by the owner as R-07, O-02 as R-27.
...
| O-02 | Paid inference for Jev through the Vercel AI Gateway (or TypeSafe's API): is it authorized, under what spend cap, with which keys and environments, and which data may be sent to the provider? Opened 2026-09-24 with drafts `009` and `012`. | Closed by the owner on 2026-09-24 (R-27): authorized for spec `012` smoke and qualification, key read from the owner's file at runtime, zero data retention, synthetic and independently labeled data only, agent-selected default cap of USD 25 per month, replaced by the owner-set caps of R-29 (USD 25 per month for production, USD 5 total for testing). |
...
| C-10 | Vercel's model page for `typesafe-ai/jev` (owner screenshot, 2026-09-24) lists two providers: TypeSafe AI (zero data retention yes, no training, 32K context) and DigitalOcean (zero data retention no). Its price read "Free" under promotional pricing ending 2026-09-25. The public page, fetched 2026-09-24, lists USD 0.042 per 1M input tokens, a 32,000-token context and no output-token price. | the work order's screenshot; `vercel.com/ai-gateway/models/jev` |
| C-11 | Five live Gateway calls on synthetic fixtures, 2026-09-24, under R-27 and R-30 (kept as spec `012` fixtures). Request: `criteria` is an object for `boolean` (`{"false": .., "true": ..}`) and `choice` (label to description), and an array of level descriptions, low to high, for `score`; an array for `boolean` is HTTP 400 `invalid_request_error`, and a `score` with 11 levels is HTTP 400 ("Too big: expected array to have <=10 items"). With `zeroDataRetention: true` on the Hobby plan: HTTP 403 `permission_denied`, no provider attempt. Response: `answers.<key>` is `{type: "boolean", probability}`, `{type: "choice", choice, probabilities{label: p}, confidence}` or `{type: "score", score, probabilities{"0".."k-1": p}, confidence}`; `model` echoes `typesafe-ai/jev`; `usage{inputTokens, outputTokens}`; `providerMetadata.typesafe.confidence{key: c}`; `providerMetadata.gateway` has `routing` (with `resolvedProvider`, `finalProvider`, `canonicalSlug`, `planningReasoning`, `modelAttempts`), `cost` and `marketCost` as decimal USD strings, and `generationId`. Observed: `cost` "0" (promotion) and `marketCost` equal to input tokens times USD 0.042 per 1M with output tokens unpriced (516 input tokens: USD 0.000021672; 2,680: USD 0.00011256); a 6,329-byte canonical state with one question used 2,680 input tokens (about 2.4 bytes per token). Total charged: USD 0. | the recorded calls |
| C-12 | spec-spine 0.26.0 is released (`spec-spine-cli` on crates.io); the pin moved from `=0.24.0` to `=0.26.0` on 2026-09-25 with the derived shards refreshed by that binary and the corpus accepted without a change. `.statecraft/environment.json` still records the scaffolded `spec-spine.toml` hash and a `pins.spec_spine` observed from `PATH`; the CI profile adoption records both through `statecraft-cli`. | `cargo search spec-spine-cli`; `make gate` with `.tooling/bin/spec-spine` 0.26.0 |
| C-13 | Gateway observations for `typesafe-ai/jev`, 2026-09-25 23:00 UTC to 2026-09-26: the public model list prices input at USD 0.000000042 per token and output at 0 (unchanged from C-10) and reports `zdr: none`; the per-model endpoint list names only DigitalOcean (`has_zdr: false`), yet every routed request resolved and finished on `typesafe-ai` (`planningReasoning`: "Provider set restricted to: typesafe-ai"). A request with `zeroDataRetention: true` got HTTP 403 `ZdrUnauthorizedError` ("only available for Pro and Enterprise plans. Current plan: hobby") with no provider attempt. The upstream answered HTTP 429 `rate_limit_exceeded` ("experiencing high demand") to all but about one request per 300 s, over more than two hours. A later run paced at 300 s still had one decision exhaust seven attempts: five HTTP 429, one HTTP 503 and one HTTP 520. Early answered requests had `cost` "0" with `marketCost` at the listed price; later answered requests charged the listed `marketCost`. | the stage 3 probe and run records (outside the repository), spec `012` implementation record |
